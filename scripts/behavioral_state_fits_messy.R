library(reshape2)
library(ggplot2)
# need a 20 min merged csv with "genotype", condition", "exp", "date","animal","stimulus"
...
y.mat <- function (n) {
  ...
  ymat$pos<-round(-(ymat$pos)/((max(ymat$pos, na.rm=TRUE))/16), digits = 1) # convert max pixels pixels? to 16 mm
...
# to get all animals entering the center and outer stripes (3):
bounds<-c(0,-3.5, -5.5, -11, -13, min(pos_beh$pos)) # set boundaries of dye stripes

# normalize for animals entering 3 stripes of cue (outer two and middle)
norm.beh.enter3<-function(df) {
  df.down1<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds[3]+2.5) & df[,'pos'] > (bounds[3]-2.5)),]
  df.down1$norm.pos<-df.down1[,'pos']-bounds[3]
  df.down2<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds[5]+2.5) & df[,'pos'] > (bounds[5]-2.5)),]
  df.down2$norm.pos<-df.down2[,'pos']-bounds[5]
  df.down<-merge(df.down1, df.down2, all=TRUE)
  df.down$norm.pos<- (-df.down[,'norm.pos'])
  
  df.up1<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds[2]+2.5) & df[,'pos'] > (bounds[2]-2.5)),]
  df.up1$norm.pos<-df.up1[,'pos']-bounds[2]
  df.up2<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds[4]+2.5) & df[,'pos'] > (bounds[4]-2.5)),]
  df.up2$norm.pos<-df.up2[,'pos']-bounds[4]
  df.up<-merge(df.up1, df.up2, all=TRUE)
  tot.mat.norm.3<-merge(df.up, df.down, all=TRUE)
  tot.mat.norm.3$norm.pos<-round((tot.mat.norm.3$norm.pos), digits = 1)
  return(tot.mat.norm.3)
}
tot.mat.norm.3<-norm.beh.enter3(tot.mat)

# normalize for animals entering two cue stripes (using dye bounds)
norm.beh.enter2<-function(df) {
  df.down1<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds[2]+2.5) & df[,'pos'] > (bounds[2]-2.5)),]
  df.down1$norm.pos<-df.down1[,'pos']-bounds[2]
  df.down2<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds[4]+2.5) & df[,'pos'] > (bounds[4]-2.5)),]
  df.down2$norm.pos<-df.down2[,'pos']-bounds[4]
  df.down<-merge(df.down1, df.down2, all=TRUE)
  df.down$norm.pos<- (-df.down[,'norm.pos'])
  
  df.up1<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds[3]+2.5) & df[,'pos'] > (bounds[3]-2.5)),]
  df.up1$norm.pos<-df.up1[,'pos']-bounds[3]
  df.up2<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds[3]+2.5) & df[,'pos'] > (bounds[3]-2.5)),]
  df.up2$norm.pos<-df.up2[,'pos']-bounds[3]
  df.up<-merge(df.up1, df.up2, all=TRUE)
  tot.mat.norm.2<-merge(df.up, df.down, all=TRUE)
  tot.mat.norm.2$norm.pos<-round((tot.mat.norm.2$norm.pos), digits = 1)
  return(tot.mat.norm.2)
}
tot.mat.norm.2<-norm.beh.enter2(tot.mat)

### to get all animals entering the middle single stripe
bounds.1<-c(0, -5, -11, -16)
norm.beh.enter<-function(df) {
  df.down<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds.1[2]+2.5) & df[,'pos'] > (bounds.1[2]-2.5)),]
  df.down$norm.pos<-df.down[,'pos']-bounds.1[2]
  df.down$norm.pos<- (-df.down[,'norm.pos'])
  
  df.up<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds.1[3]+2.5) & df[,'pos'] > (bounds.1[3]-2.5)),]
  df.up$norm.pos<-df.up[,'pos']-bounds.1[3]
  tot.mat.norm.1<-left_join(df.down, df.up)
  #tot.mat.norm.1<-merge(df.up, df.down, all=TRUE)
  tot.mat.norm.1$norm.pos<-round((tot.mat.norm.1$norm.pos), digits = 1)
  return(tot.mat.norm.1)
}
tot.mat.norm.enter<-norm.beh.enter(tot.mat)

#for animals exiting the single middle stripe:
norm.beh.exit<-function(df) {
  df.down<-df[df[,'direction'] == "down" & (df[,'pos'] < (bounds.1[3]+2.5) & df[,'pos'] > (bounds.1[3]-2.5)),]
  df.down$norm.pos<-df.down[,'pos']-bounds.1[3]
  df.down$norm.pos<- (-df.down[,'norm.pos'])
  
  df.up<-df[df[,'direction'] == "up" & (df[,'pos'] < (bounds.1[2]+2.5) & df[,'pos'] > (bounds.1[2]-2.5)),]
  df.up$norm.pos<-df.up[,'pos']-bounds.1[2]
  tot.mat.norm.1<-left_join(df.down, df.up)
  #tot.mat.norm.1<-merge(df.up, df.down, all=TRUE)
  tot.mat.norm.1$norm.pos<-round((tot.mat.norm.1$norm.pos), digits = 1)
  return(tot.mat.norm.1)
}
tot.mat.norm.exit<-norm.beh.exit(tot.mat)



# # get post-reversal behavior (as column now but should figure better way) - skip this part for now
# library(data.table)
...
#pos_beh<-aggregate(cbind(Rev, For, Pir, Curve, Pau) ~ genotype + pos + bin + condition + exp + direction, data = tot.mat.norm.1, FUN = mean)
#pos_bin_beh<-aggregate(cbind(Rev, For, Pir, Curve, Pau) ~ pos.bin + bin + condition + exp + wormID + direction, data = tot.mat.norm.1, FUN = mean)

norm_timbin_beh<-aggregate(cbind(Pir, For, Rev, Curve) ~ genotype + norm.pos + bin + condition + wormID, data = tot.mat.norm.exit, FUN=mean)
norm_pos_beh<-aggregate(cbind(Pir, For, Rev, Curve) ~ genotype + norm.pos + condition + wormID, data = tot.mat.norm.1, FUN = mean)
...
plot1<-ggplot(norm_timbin_beh, aes(x=norm.pos)) +
...
library(splines)
library(scales)
plot3<-ggplot(subset(norm_timbin_beh, direction == "up" | direction == "down"), aes(x=norm.pos)) +
  ...
  geom_smooth(aes(y=Pir, linetype=condition), method = loess, span = 0.5, alpha = 0.15, colour="blue") +
  #geom_smooth(aes(y=omR, linetype=condition), colour="magenta") +
  geom_smooth(aes(y=Rev, linetype=condition), method = loess, span = 0.5, alpha=0.15, colour="red") +
  geom_smooth(aes(y=Curve, linetype=condition), method = loess, span = 0.5, alpha=0.15,colour="orange") +
  #geom_smooth(aes(y=Pau, linetype=condition), level=0.99,colour="yellow") + 
  geom_smooth(aes(y=For, linetype=condition), method = loess, span = 0.5, alpha = 0.15, colour="green") +
  facet_grid(genotype~.) 
  #coord_cartesian(ylim=c(0, 1))
plot3
...
plot4<-ggplot(norm_pos_beh,
  ...
  geom_smooth(aes(y=Pir, linetype=condition), method=loess, span = 0.5, alpha = 0.1, colour="blue") +
  geom_smooth(aes(y=Rev, linetype = condition), method=loess, span = 0.5,alpha=0.1,colour="red") + 
  geom_smooth(aes(y=Curve, linetype = condition), method=loess, span = 0.5,alpha=0.1, colour="purple") + 
  geom_smooth(aes(y=For, linetype = condition), method=loess, span = 0.5,alpha=0.1, colour = "darkgreen") +
  coord_cartesian(xlim=c(-1.5,1.5), ylim=c(-.05, 0.8)) + theme_my
  #facet_wrap(~bin)
  plot4
